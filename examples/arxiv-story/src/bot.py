...
from utils.arxiv import get_arxiv_content, truncate_content
...
async def main(room_url, token=None, arxiv_url=None):
    async with aiohttp.ClientSession() as session:
        # Get paper content if URL provided
        paper_content = ""
        if arxiv_url:
            logger.debug(f"Fetching arXiv paper content from: {arxiv_url}")
            paper_content = await get_arxiv_content(arxiv_url, session)
            paper_content = truncate_content(paper_content, model_name="gemini-pro")
        ...
        # Update base prompt with paper content if available
        base_prompt = LLM_BASE_PROMPT.copy()
        if paper_content:
            base_prompt["content"] = base_prompt["content"].format(paper_content=paper_content)

        message_history = [base_prompt]
        story_pages = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arxiv", type=str, help="arXiv paper URL", default=None)
    config = parser.parse_args()

    asyncio.run(main(config.u, config.t, config.arxiv))
