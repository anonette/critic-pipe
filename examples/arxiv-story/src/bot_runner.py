...
async def start_bot(request: Request) -> JSONResponse:
    ...
    if os.getenv("RUN_AS_VM", False):
        try:
            await virtualize_bot(room.url, token, arxiv_url)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to spawn VM: {e}")
    else:
        try:
            cmd = f"python -m bot -u {room.url} -t {token}"
            if arxiv_url:
                cmd += f" --arxiv {arxiv_url}"
            subprocess.Popen(
                [cmd],
                shell=True,
                bufsize=1,
...
async def virtualize_bot(room_url: str, token: str, arxiv_url: Optional[str] = None):
